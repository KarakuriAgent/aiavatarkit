...
| Feature | Description |
|---------|-------------|
| **cache_dir** | Directory to store cached audio files. Set to enable caching |
| **cache_ext** | File extension for cache files (default: `wav`, Google: `pcm`) |
| **Automatic key generation** | SHA-256 hash of request parameters ensures unique cache per input |
| **All TTS supported** | Works with Azure, OpenAI, Google, Voicevox, and InstantSynthesizer |

```python
# Enable caching by setting cache_dir
