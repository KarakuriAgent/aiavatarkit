...
"""
    ...
    @abstractmethod
    async def get_llm_stream_response(self, context_id: str, user_id: str, messages: List[dict], system_prompt_params: Dict[str, any] = None, tools: List[Dict[str, any]] = None, inline_llm_params: Dict[str, any] = None, session_id: str = None, channel: str = None, request_start_callback: Callable[[float], None] = None) -> AsyncGenerator[LLMResponse, None]:
        pass
    ...
    async def chat_stream(self, context_id: str, user_id: str, text: str, files: List[Dict[str, str]] = None, system_prompt_params: Dict[str, any] = None, inline_llm_params: Dict[str, any] = None, session_id: str = None, channel: str = None, request_start_callback: Callable[[float], None] = None) -> AsyncGenerator[LLMResponse, None]:
        if self._print_chat:
        ...
        async for chunk in self.get_llm_stream_response(context_id, user_id, messages, system_prompt_params, inline_llm_params=inline_llm_params, session_id=session_id, channel=channel, request_start_callback=request_start_callback):
            if chunk.error_info:
                if self._on_error:
